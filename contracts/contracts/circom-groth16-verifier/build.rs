...
/// Generate zero-filled placeholder constants when no VK file is provided.
/// Allows `cargo check` and unit tests that supply a VK directly to work
/// before a real circuit has been compiled.
fn placeholder_vk_rs() -> String {
    let zero64 = fmt_bytes(&[0u8; 64]);
    let zero128 = fmt_bytes(&[0u8; 128]);
    // 11 public inputs + 1 constant term = 12 IC points (matches the spend circuit)
    let ic_len = 12usize;
    let ic_items = vec![zero64.clone(); ic_len];
    let mut out = String::new();
    writeln!(out, "// Auto-generated placeholder by build.rs — set VERIFIER_VK_JSON to embed real keys.")
        .expect("infallible write to String");
    writeln!(out, "const VK_ALPHA_G1: [u8; 64] = {};", zero64).expect("infallible write to String");
    writeln!(out, "const VK_BETA_G2: [u8; 128] = {};", zero128).expect("infallible write to String");
    writeln!(out, "const VK_GAMMA_G2: [u8; 128] = {};", zero128).expect("infallible write to String");
    writeln!(out, "const VK_DELTA_G2: [u8; 128] = {};", zero128).expect("infallible write to String");
    writeln!(
        out,
