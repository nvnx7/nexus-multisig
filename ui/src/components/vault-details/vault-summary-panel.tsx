...
export interface VaultSummaryPanelProps {
  vaultAddress: string;
  balance: bigint | null;
  ...
  onCopyAddress: () => void;
  userAddress?: string;
  /** Optional slot pinned to the bottom of the panel (e.g. action buttons). */
...
/**
 * The dark-green vault identity panel shared by the vault details page and the
 * signing ceremony page. Renders the vault title, shielded address, balance and
 * signing policy. Pass action buttons (or anything) via `children` to pin them
      ...
      <Flex direction="column" gap={0.5} flexShrink={0}>
        <Flex align="center" gap={3}>
          ...
          <Box
            ...
            flexShrink={0}
          >
            <Text fontFamily="mono" fontSize="2xs" color="brand.text" letterSpacing="0.02em">
        ...
        {balanceLoading ? (
          ...
          <>
            <Flex align="baseline" gap={2} mt={0.5}>
              ...
              <Text fontFamily="mono" fontSize="sm" color="brand.text" fontWeight="medium">
                XLM
              </Text>
            </Flex>
            {xlmPrice != null && balance !== null && (
              <Text fontFamily="mono" fontSize="xs" color="brand.text" opacity={0.7} mt={1}>
                {"≈ $"}
              ...
              members.map((m, idx) => (
                <Flex key={m.address} align="flex-start" gap={2}>
                  ...
                  <Text
                    fontFamily="mono"
                    fontSize="xs"
                    color={m.address === userAddress ? "white" : "rgba(255,255,255,0.65)"}
                    wordBreak="break-all"
                    lineHeight="1.6"
                  >
                    {m.address}
